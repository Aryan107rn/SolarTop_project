export default function Navbar() {
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-8 py-4 backdrop-blur-md bg-[#080f08]/60 border-b border-white/5">
        ...
        {['Services', 'Projects', 'About', 'Contact'].map(link => (
          <li key={link}>
            <a
              ...
              className="px-4 py-2 rounded-full text-[#7a9b7e] hover:bg-[#c5f135] hover:text-[#1a2e1a] transition-all duration-300"
            >
              {link}
            </a>
          </li>
      ...
      <a
        href="#contact"
        className="text-xs font-bold tracking-widest uppercase bg-[#c5f135] text-[#080f08] px-5 py-2 rounded-full hover:opacity-80 transition-opacity"
      >
        Get Started
      </a>
